...
public class ChRoomServlet extends HttpServlet{
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
        ...
        ChatRoom  chatroom = getRoom(request, response);
        if(chatroom == null){
            return;
        }
    ...
    private ChatRoom  getRoom(HttpServletRequest request, HttpServletResponse response)
        throws IOException, ServletException{
		HttpSession httpsession = request.getSession(true);
        PrintWriter printwriter = response.getWriter();
        ...
        ChatRoom  chatroom = (ChatRoom )hashmap.get(s);
    	if(chatroom == null){
            error(request, response);
            return null;
        }
    ...
    private void messageFrame(HttpServletResponse response, ChatRoom  chatroom)
        throws IOException{
		PrintWriter printwriter = response.getWriter();
		printwriter.println("<html>");
        printwriter.println("<head><title>" + chatroom.getChatRoomName() + "</title></head>");
    ...
    private void writeMessage(PrintWriter printwriter, ChatRoom  chatroom, String s){
	...
	printwriter.println("<tr><td valign=top>");
        printwriter.println("Chat-Room Name: " + chatroom.getChatRoomName() + "<br>You are: " + s + "<br><br><br>");
        if(chatroom.getRoomSize() == 0){
